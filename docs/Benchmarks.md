...
| Clients (`--clients`) | TPS   | Avg Latency |
| --------------------- | ----- | ----------- |
| 1                     | ~1100 | ~0.9 ms     |
| 5                     | ~3600 | ~1.6 ms     |
| 10                    | ~4900 | ~2.5 ms     |
...
| Scale (`--scale`) | Dataset Size | TPS (20 clients) |
| ----------------- | ------------ | ---------------- |
| 1                 | 100k rows    | ~5350            |
| 5                 | 500k rows    | ~3900            |
| 10                | 1M rows      | ~3500            |
...
pgbench demonstrates predictable scaling behavior. With this foundation, the telemetry agent will measure:
...
- Add threads support to the CLI
...
# Run benchmark with default settings (60s, 10 clients)
./telemetryctl benchmark local
```
...
```bash
# Run 2-minute benchmark with 20 clients
./telemetryctl benchmark local --duration 120 --clients 20

# Run benchmark with larger dataset
./telemetryctl benchmark local --scale 10 --duration 60 --clients 20

# Run with custom progress interval
./telemetryctl benchmark local --duration 300 --clients 50 --progress 10
```
...
./telemetryctl benchmark local --config configs/custom.yaml
```
